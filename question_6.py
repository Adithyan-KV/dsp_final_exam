...
def main():
    ...
    EAHN_2 = get_eahn(speech_signal)
    # for 0 db noise
    noisy_signal = add_noise_to_signal(EAHN_1, speech_signal, 5)
    fft_signal = fft.fft(noisy_signal)
    freq = fft.fftfreq(len(speech_signal),d=1/sampling_rate)
    power_spectrum = fft_signal*np.conjugate(fft_signal)
    # plt.plot(freq, np.abs(power_spectrum))
    # plt.scatter(np.arange(-3600,3600,600),np.zeros(12),color='red')
    # plt.show()
    power_noise = 0
    for harmonic_freq in range(600,4000,600):
        power_noise += np.abs(np.sum(power_spectrum[harmonic_freq-2:harmonic_freq+3]))
    power_signal = np.abs(np.sum(power_spectrum)-2*power_noise)
    ...
    # UAHN_2 = get_uahn(speech_signal)
    # # for 0 db noise
    # noisy_signal = add_noise_to_signal(UAHN_1, speech_signal, 15)
    # fft_signal = fft.fft(noisy_signal)
    # # freq = fft.fftfreq(len(noisy_signal),d=1/sampling_rate)
    # power_spectrum = fft_signal*np.conjugate(fft_signal)
    # # plt.plot(freq, np.abs(power_spectrum))
    # # plt.scatter(np.arange(120,1000,120),np.zeros(8),color='red')
    # # plt.show()
    # power_noise = 0
    # for harmonic_freq in range(600,4000,600):
    #     power_noise += np.abs(np.sum(power_spectrum[harmonic_freq-1:harmonic_freq+1]))
    # power_signal = np.abs(np.sum(power_spectrum)-2*power_noise)
    # snr = 10*np.log10(power_signal/power_noise)
    # print(snr)
